# Django API backend for TMI project

This repo contains a django web service project running on docker that have the following services:
...
Note: you can run this project on a host machine without docker, see -Old fashion install (Host)
...
## Old fashion install (Host)
First of all, install pipenv so you can use the specified python version (check out .python-version). 

Then, run `pip install pipenv` to install it's pip's successor: pipenv. Then install dependencies by running: 
